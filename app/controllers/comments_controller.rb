  ...
  def create
    if request.xhr?
      ...
      # render json: {content: @comment.content, userimage: current_user.image_url,username:current_user.name, timeago: comment_time}
        render :"comment/_new", :layout => false, :locals => {facebook_id: current_user.facebook_id, content: @comment.content, userimage: current_user.image_url,username:current_user.name, timeago: comment_time}
    else
